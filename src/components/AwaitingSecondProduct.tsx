...
export const AwaitingSecondProduct: React.FC<AwaitingSecondProductProps> = ({
    ...
    return (
        <div className="absolute top-0 left-0 right-0 z-10 bg-blue-50 border-b-2 border-blue-200 p-2 sm:p-3">
            {/* Mobile Layout - Single compact row */}
            <div className="flex items-center justify-between sm:hidden">
                <div className="flex items-center space-x-1 min-w-0 flex-1">
                    <div className="w-1.5 h-1.5 bg-blue-500 rounded-full flex-shrink-0"></div>
                    <span className="text-xs text-blue-800 truncate">vs {firstProduct.name}</span>
                    <div className={`px-1 py-0.5 rounded text-xs font-medium ${FOOD_CATEGORIES[firstProduct.category].bgColor} ${FOOD_CATEGORIES[firstProduct.category].color} flex-shrink-0`}>
                ...
                <Button
                    ...
                    variant="outline"
                    size="icon"
                    className="h-6 w-6 border-blue-300 hover:bg-blue-100 flex-shrink-0 ml-2"
                >
                    <X className="w-3 h-3" />
                </Button>
            </div>

            {/* Desktop Layout - Two rows with more space */}
